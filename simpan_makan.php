...
if(mysqli_num_rows($query_cek_nim)>0)
{
	...
	$nim = $row_cek_nim['nim'];
	$cek_duplicate = "SELECT * FROM ".$prefix."t_makan WHERE nim = '$nim'";
	$sql_insert = "INSERT INTO ".$prefix."t_makan (nim) VALUES ('$nim')";
	$query_duplicate = mysqli_query($conn, $cek_duplicate);
	if(mysqli_num_rows($query_duplicate)==0)
	{
		mysqli_query($conn, $sql_insert);
		header("Location: makan.php?p=1&nim=".$nim);
	}
	else
	{
		header("Location: makan.php?p=0&nim=".$nim);	
	}
}
else
{
