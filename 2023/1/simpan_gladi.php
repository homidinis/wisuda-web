...
if(mysqli_num_rows($query_cek_nim)>0)
{
	...
	$nim = $row_cek_nim['nim'];
	$cek_duplicate = "SELECT * FROM ".$prefix."t_gladi WHERE nim = '$nim'";
	$sql_insert = "INSERT INTO ".$prefix."t_gladi (nim) VALUES ('$nim')";
	// echo $sql_insert;
	$query_duplicate = mysqli_query($conn, $cek_duplicate);
	if(mysqli_num_rows($query_duplicate)==0)
	{
		mysqli_query($conn, $sql_insert);
		header("Location: gladi.php?p=1&nim=".$nim);
	}
	else
	{
		header("Location: gladi.php?p=0&nim=".$nim);
	}
}
else
{
	header("Location: gladi.php?p=0");	
}
?>