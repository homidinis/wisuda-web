...
function simpanNilai(id)
{
    ...
    var idx = document.getElementById("idx"+id).innerHTML;
    var klmpk = document.getElementById('klmpk'+id).value;
    var pos = document.getElementById("judul").innerHTML; 
    // alert(nilai + " " + idx + " " + klmpk + " " + pos);
    if(nilai>1000 || nilai<0){
        alert("NILAI TIDAK BOLEH LEBIH DARI 1000 / KURANG DARI 0");
        return false;
    }
	$.ajax({
		type: "POST",
...
<?php
    ...
    for ($i=1; $i < 17; $i++) {
    	...
    	{
    		$j ++;
    		echo "<tr><td colspan='4'>Kloter ".$j."</td></tr>";
    	} 
        $get_idx = "SELECT * FROM t_main WHERE pos = '$id' AND idx = '$i'";
        $query_idx = mysqli_query($conn, $get_idx);
        $row_idx = mysqli_fetch_array($query_idx);
        $klmpk = $row_idx['id_kelompok'];
        $nilai = $row_idx['nilai'];
        $kode_klmpok = $kode[$j];
        echo "<tr>";
        echo "<td id='idx".$i."' style='text-align:center;'>".$i."</td>";
        echo "<td>";
            echo "<select class='form-control' id='klmpk".$i."'>";
                echo "<option value=''>--Pilih Kelompok--</option>";
                        $sql_klmpk = "SELECT * FROM t_kelompok WHERE kelompok LIKE '%$kode_klmpok%'";
                        $query_klmpk = mysqli_query($conn, $sql_klmpk);
                        while ($row_klmpk = mysqli_fetch_array($query_klmpk)) {
                            if($row_klmpk['id'] == $klmpk)
                                echo "<option value='".$row_klmpk['id']."' selected>K ".$row_klmpk['kelompok_besar']."</option>";
                            else
                                echo "<option value='".$row_klmpk['id']."'>K ".$row_klmpk['kelompok_besar']."</option>";
                        }
            echo "</select>";
        echo "</td>";
        echo "<td><input type='number' class='form-control' id='nilai".$i."' value='".$nilai."' min='1' max='1000'></td>";
        echo "<td><button class='form-control' onclick='simpanNilai(".$i.")'>Simpan</button></td>";
        echo "</tr>";
    }
?>
    </tbody>
